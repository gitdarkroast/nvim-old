-- Used config from: EdenEast feline config
...
local u = require("config.feline.util")
require("config.feline.colors")
...
local get_diag = function(err)
	local opts = {
		severity = err,
	}
	local tbl = vim.diagnostic.get(0, opts)
	-- determine the numbers of entries in the table
	local count = 0
